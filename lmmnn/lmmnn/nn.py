...
def reg_nn_lmm(base_model, optimizer, X_train, X_val, X_test, y_train, y_val, y_test, qs, q_spatial, x_cols, batch_size, epochs, patience, n_neurons, dropout, activation,
    ...
    if mode == 'spatial_embedded':
        ...
        X_val_z_cols = [X_val[z_col] for z_col in z_cols]
        X_test_z_cols = [X_test[z_col] for z_col in z_cols]

    history = model.fit([X_train[x_cols], y_train] + X_train_z_cols, None,
                        batch_size=batch_size, epochs=epochs, validation_data=[[X_val[x_cols], y_val] + X_val_z_cols],
    ...
    y_pred_tr = model.predict(
        [X_train[x_cols], y_train] + X_train_z_cols, batch_size = batch_size).reshape(X_train.shape[0])
    ...
    if mode in ['intercepts', 'glmm', 'spatial', 'spatial_and_categoricals']:
        if Z_non_linear or len(qs) > 1 or mode == 'spatial_and_categoricals':
            ...
            y_pred = model.predict([X_test[x_cols], dummy_y_test] + X_test_z_cols, batch_size = batch_size).reshape(
    ...
    return y_pred, (sig2e_est, list(sig2b_ests), list(sig2b_spatial_ests)), list(rho_ests), list(weibull_ests), len(history.history['loss']), b_hat, model, y_pred_tr
